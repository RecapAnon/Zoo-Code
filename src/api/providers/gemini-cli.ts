...
type GeminiCliModel = ReturnType<GeminiCliHandler["getModel"]>
...
export class GeminiCliHandler extends BaseProvider implements SingleCompletionHandler {
	...
	constructor(options: GeminiCliHandlerOptions) {
		...
		this.sessionId = uuidv7() // Initialize Cli2 client
	}
	...
	private getProjectId(): string {
		...
		if (!projectId) {
			throw new Error(t("common:errors.geminiCli.oAuthLoadFailed"))
		}
		return projectId
	}
		...
		try {
			for await (const jsonData of this.parseSSEStream(body)) {
				...
				if (response?.error || response?.message) {
					throw new Error(
						t("common:errors.geminiCli.apiError", {
			...
			// Gemini Cli can occasionally return STOP with no text/tool output,
	...
	async *createMessage(
		...
		await geminiOAuthManager.ensureAuthenticated({ path: this.options.geminiCliPath })
	...
	override getModel() {
		...
		if (id.endsWith(":thinking")) {
			id = id.replace(":thinking", "") as GeminiCliModelId
		}
